...
function readLeads() {
    try {
        ...
        console.log(`Read ${leads.length} leads from ${config.leadsFile}`);
        return leads;
    } catch (error) {
        console.log(`Failed to read leads file: ${error.message}`);
        return [];
    }
}
...
function saveLeads(leads) {
    try {
        ...
        console.log(`Leads file saved successfully`);
        return true;
    } catch (error) {
        console.log(`Failed to save leads file: ${error.message}`);
        return false;
    }
}
...
function needsContact(lead) {
    ...
    // Never contact unsubscribed or converted or stopped leads
    ...
    // Check timing based on current stage
...
function updateLeadAfterContact(lead) {
    ...
    console.log(`${lead.name} stage updated to: ${lead.stage}`);
    return lead;
}
...
function markAsConverted(lead) {
    ...
    console.log(`${lead.name} marked as CONVERTED`);
    return lead;
}
...
function markAsUnsubscribed(lead) {
    ...
    console.log(`${lead.name} marked as UNSUBSCRIBED`);
    return lead;
}
...
function getLeadsToContact() {
    ...
    console.log(`${leadsToContact.length} leads need contact today`);
